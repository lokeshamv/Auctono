...
export const AddCarForm = () => {
  ...
  useEffect(() => {
    if (!processImageResult) return;

    if (processImageResult.success) {
      const carDetails = processImageResult.data || {};

      // Safely apply AI results to the form with fallbacks
      try {
        setValue("make", carDetails.make || "");
        setValue("model", carDetails.model || "");
        setValue("year", String(carDetails.year || new Date().getFullYear()));
        ...
        console.error("Error applying AI results to form:", e);
      }

      // Add the image to the uploaded images preview (if available)
      if (uploadedAiImage) {
        try {
          ...
          console.error("Failed to read uploaded AI image:", e);
        }
      }
      ...
      // AI returned an error or parsing failed
      const errMsg = processImageResult.error || "AI could not extract details";
      toast.error(errMsg);
    }
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-6">
                ...
                {processImageResult && (
                  <div className="bg-white border rounded-md p-3 mt-4 overflow-auto text-xs">
                    <h4 className="font-medium mb-2">AI Debug Output</h4>
                    <pre className="whitespace-pre-wrap break-words">{JSON.stringify(processImageResult, null, 2)}</pre>
                  </div>
                )}
              </div>
            </CardContent>
